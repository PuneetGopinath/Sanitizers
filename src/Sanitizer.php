...
class Sanitizer
{
     ...
     * @param string|null $depreciated If the warning is for something depreciated, mention it here
    ...
    private function warn($msg, $depreciated = null, $deprecatedType = "function")
    {
        ...
        if (!empty($depreciated)) {
            $message .= "Using depreciated " . $deprecatedType . $depreciated;
            $message .= ", " . $msg;
            error_log($message);
            return $message;
        }
    ...
    public function set($case, $value = "default")
    {
        ...
        switch ($case) {
            ...
            default:
                if ($this->exceptions) {
                    throw new \Exception($this->fatal(
                        "Invalid config key: $case with value: $value"
                    ));
                }
                return false;
                break;
        ...
        if (
            ...
            if (
                ...
                $msg = $this->fatal(
                    "If you set preventXSS as true" .
                    " then you should also set encoding to \"UTF-8\""
                );
                if ($this->logger) {
    ...
    {
        $text = strip_tags($this->stripTagsContent((string)$text));
    ...
    public function sanitize($type, $text, $trim = true, $htmlspecialchars = true, $alpha_num = false, $ucwords = false)
    {
        ...
        if (empty($type)) {
            ...
            if ($type === "string") {//If $type is not given, php detects it as a string
    ...
    {
        $this->warn(
            "use sanitize function with type param as \"text\"" .
            "(see functions in docs for more info)",
            "NonNumericText"
        );
        $text = $this->sanitize(
            "text",
     ...
     * @param bool   $invert Do you want to change param 2 to `The disallowed html tags.`?
